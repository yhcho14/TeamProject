...
public class PlayerController : MonoBehaviour
{
    ...
    private float jumpTimeCounter;  // 점프 시간 측정
    public float jumpTime; // 점프 길이
    private bool isJumping; // 점프 중인지 아닌지 체크
    ...
    private void Update()  // 매 프레임마다 호출
    {
        ...
        if(isGrounded == true && Input.GetKeyDown(KeyCode.Space)) // 점프 실행
        {
            isJumping = true;
